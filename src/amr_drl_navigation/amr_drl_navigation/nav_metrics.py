...
class Navigation_Metrics(Node):
    def __init__(self, logdir):
        ...
        """ """
        super().__init__("amr_nav_metrics")
        # self.params = params
        ...
        self.pose_sub_ = self.create_subscription(TFMessage, "/model/amr/pose", self.pose_callback, 10)

        self.odom_sub_= self.create_subscription(Odometry, "/amr_controller/odom", self.odom_callback, 10)
    ...
    def pose_callback(self, msg: TFMessage):
        ...
        for t in msg.transforms:
            if (t.child_frame_id == self.params["robot_name"] and t.header.frame_id in ("empty_world", "default")):
                self.latest_pose = t.transform
    ...
    def acquire_data(self):
        ...
        robot_pose = [
            self.latest_pose.translation.x,
            self.latest_pose.translation.y,
            quat_to_euler(self.latest_pose.rotation.z, self.latest_pose.rotation.w),
        ]

        if self.latest_twist:
            velocity = [
                np.hypot(self.latest_twist.linear.x, self.latest_twist.linear.y),
