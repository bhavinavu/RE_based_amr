...
from amr_drl_navigation.sensors import Sensors
from amr_drl_navigation.env_utils import *
from amr_drl_navigation.nav_metrics import Navigation_Metrics
...
class Pic4rlEnvironmentLidar(Node):
    def __init__(self):
        ...
        self.declare_parameter("package_name", "amr_drl_navigation")
        ...
        self.entity_path = os.path.join(
            get_package_share_directory("amr_drl_navigation"), "models/goal_box/model.sdf"
        )
        ...
        self.robot_urdf_xml = subprocess.check_output(["xacro", xacro_path]).decode("utf-8")
        self.goal_entity = os.path.join(get_package_share_directory("amr_drl_navigation"), 'models', 
            'goal_box', 'model.sdf')
        
        with open(self.goal_entity, "r") as f:
        ...
        self.cmd_vel_pub = self.create_publisher(Twist, "/amr_controller/cmd_vel_unstamped", qos)

        self.reset_world_client = self.create_client(Empty, "reset_world")
        self.pause_physics_client = self.create_client(Empty, "pause_physics")
        self.unpause_physics_client = self.create_client(Empty, "unpause_physics")
        self.control_client = self.create_client(ControlWorld, "/world/default/control")
    ...
    def send_action(self, twist):
        """ """
        while self.count_subscribers("amr_controller/cmd_vel") == 0:
    ...
    def new_episode(self):
        ...
        while not self.control_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn("Control service not available, waiting again...")
    ...
    def respawn_goal(self, index):
        ...
        req.entity_factory.name = "goal"
        req.entity_factory.sdf = self.goal_sdf
        req.entity_factory.pose.position.x = float(x)
        req.entity_factory.pose.position.y = float(y)
        req.entity_factory.pose.position.z = 0.05
        future = spawn_client.call_async(req)
        rclpy.spin_until_future_complete(self, future)
        result = future.result()
        if not result.success:
            self.get_logger().warn(f"Failed to set robot pose: {result.status_message}")
    ...
    def spawn_controllers(self):
        ...
        # spawn wheel controller
        proc2 = subprocess.Popen([
            "ros2", "run", "controller_manager", "spawner",
            "amr_controller", "--controller-manager", "/controller_manager"
        ])
        proc2.wait()
    ...
    def respawn_robot(self, index):
        ...
        spawn_goal_client = self.create_client(SpawnEntity, "/world/default/create")

        req = SpawnEntity.Request()
        req.entity_factory.name = self.robot_name
        req.entity_factory.sdf = self.robot_urdf_xml
        req.entity_factory.pose.position.x = x
        req.entity_factory.pose.position.y = y
        req.entity_factory.pose.position.z = 0.07
        req.entity_factory.pose.orientation.z = qz
        req.entity_factory.pose.orientation.w = qw
        future = spawn_goal_client.call_async(req)
        rclpy.spin_until_future_complete(self, future)
        result = future.result()
        if not result.success:
            self.get_logger().warn(f"Failed to set robot pose: {result.status_message}")
